...
const pageWrap = { maxWidth: '760px', margin: '0 auto', textAlign: 'left' };
const titleStyle = { color: '#111111', fontSize: '1.9rem', lineHeight: 1.3, marginBottom: '6px', textAlign: 'left' };
const metaStyle = { color: '#111111', marginBottom: '14px', textAlign: 'left' };
const boxStyle = { border: '2px solid #111111', borderTop: '6px solid #FFEE00', borderRadius: '14px', padding: '18px 20px', marginBottom: '18px', background: '#ffffff', textAlign: 'left' };
const headingRow = { display: 'flex', alignItems: 'center', gap: '8px', color: '#111111', margin: '0 0 10px 0', fontSize: '1.1rem', textAlign: 'left' };
const darkText = { color: '#111111', margin: 0, textAlign: 'left' };
const listStyle = { margin: 0, paddingLeft: '20px', textAlign: 'left', color: '#111111' };
...
function CourseDetail() {
  ...
  if (loading) return <p className="course-detail-status">Loading course…</p>;
  if (!course) return <p className="course-detail-status">Course not found.</p>;
    ...
    <div className="course-detail" style={pageWrap}>
      ...
      <header style={{ marginBottom: '20px', textAlign: 'left' }}>
        ...
        <div className="no-print" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <BookmarkButton institution={{ id: course.course_id, name: course.name, location: institution?.name, qualification_type: course.qualification_type, institution_id: course.institution_id }} />
          <PrintButton />
          {institution && (
      ...
      <SectionBox icon={Building2} title="Institution">
        ...
        {campuses.length > 0 && (
          <div style={{ marginTop: '10px' }}>
            <p style={{ display: 'flex', alignItems: 'center', gap: '6px', fontWeight: 600, margin: '0 0 6px 0', color: '#111111', textAlign: 'left' }}><MapPin size={16} strokeWidth={2} /> Campuses</p>
            <ul style={listStyle}>
      ...
      <SectionBox icon={Clock} title="Mode of Study & Duration">
        {modeDurations.length === 0 ? (
          <p style={darkText}>Not specified. Check the official source for details.</p>
        ) : (
          <ul style={listStyle}>
            {modeDurations.map((m, i) => (
              <li key={i}>{MODE_LABELS[m.mode] || m.mode}: {m.duration}</li>
            ))}
          </ul>
        )}
      </SectionBox>
      ...
      <SectionBox icon={ClipboardCheck} title="Admission Requirements">
        ...
        {otherRequirements.length > 0 ? (
          <ul style={{ ...listStyle, marginTop: course.minimum_aps ? '8px' : 0 }}>
      ...
      <div style={boxStyle}>
        ...
        <p style={{ ...darkText, whiteSpace: 'pre-wrap' }}>{course.description}</p>
        {course.source_link && (
